...
const fixVersion = function(issue) {
  const field = 'Fix Version/s'
  return historySeries(issue, field, issue => {
    return issue[field]
  })
}

const epicLink = function(issue) {
  const field = 'Epic Link'
  return historySeries(issue, field, issue => {
    return issue[field]
  })
}
...
const addAllSeries = function(issue) {
  return addHistorySeries(issue, status, fixVersion, epicLink)
}
...
const historySeries = function (issue, fieldName, currentFieldValueMap) {
  ...
  const lastUpdated = issue.Updated

  // find current value of given field
  const currentValue = currentFieldValueMap(issue)
  ...
  let history = fieldChanges.map(change => {
    if(!initialValue) {
      initialValue = change.fromString
    }
    return {
